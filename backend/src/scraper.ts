...
import type { SchoolCatalog, SchoolListEntry, ScrapeResult, ScrapedSchoolData, SchoolSyncStatus } from "./types.js";
...
async function ensureSchoolListFile(): Promise<void> {
...
async function ensureOutputFile(): Promise<void> {
  ...
  await ensureFileExists(
    OUTPUT_FILE,
    JSON.stringify({ updatedAt: new Date().toISOString(), schools: [] }, null, 2)
  );
}
...
async function saveCatalog(catalog: SchoolCatalog): Promise<void> {
  await ensureDataDirectory();
  await writeFile(OUTPUT_FILE, JSON.stringify(catalog, null, 2), "utf8");
}

async function persistSchoolToSupabase(school: ScrapedSchoolData, syncStatus: SchoolSyncStatus): Promise<void> {
  try {
...
async function cleanupDebugHtmlFiles(): Promise<number> {
  if (!DEBUG_CLEANUP_ENABLED) return 0;
  ...
  const byHash = new Map<string, Array<{ filePath: string; mtimeMs: number }>>();
...
function createSlug(value: string | undefined, fallback: string): string {
...
async function loadFrontendSchoolAllowlist(): Promise<Set<string>> {
  ...
  for (const match of raw.matchAll(/slug:\s*"([^"]+)"/g)) {
    ...
    if (slug) allowlist.add(slug);
  }

  for (const match of raw.matchAll(/name:\s*"([^"]+)"/g)) {
    const normalizedName = normalizeToken(match[1]);
    if (normalizedName) allowlist.add(normalizedName);
  }

  return allowlist;
}

function shouldKeepEntry(entry: SchoolListEntry, allowlist: Set<string>): boolean {
...
function maybePlaceholderPage($: CheerioAPI): boolean {
  const checks = [
    $("h1.school-name").text(),
...
function selectPanelValue($: CheerioAPI, label: string): string | undefined {
  const normalizedLabel = label.trim().replace(/:$/, "");
  const paragraph = $(".panel-body p").filter((_, el: any) => {
...
function buildSyncStatus(entry: SchoolListEntry, success: boolean, message: string, sourceUrl: string): SchoolSyncStatus {
  return {
    npsn: entry.npsn,
    slug: entry.slug,
...
function resolveSchoolIdentifier(entry: SchoolListEntry): string {
...
async function navigateToSchoolViaSearch(page: any, entry: SchoolListEntry): Promise<string | undefined> {
  ...
  for (const query of queries) {
    const filled = await page.evaluate((value: string, selectors: string[]) => {
      const normalizeVisible = (element: Element | null): element is HTMLElement => {
      ...
      const target = selectors
        ...
        .find((element): element is HTMLInputElement | HTMLTextAreaElement => Boolean(element) && normalizeVisible(element) && (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement));
    ...
    const resultLink = await page.evaluate((npsn: string, slug: string) => {
...
async function fetchSchoolPageWithBrowser(url: string, entry?: SchoolListEntry): Promise<string> {
...
async function fetchSchoolPage(npsn: string, entry?: SchoolListEntry): Promise<string> {
...
async function fetchSchoolPageWithRetry(npsn: string, attempts = 3, entry?: SchoolListEntry): Promise<string> {
...
export function parseSchoolFromHtml(html: string, entry: SchoolListEntry): ScrapedSchoolData {
  const $ = cheerio.load(html);
  ...
  let yearEstablished: string | undefined = undefined;
  ...
  if (profil && profil.length) {
    profil.find(".panel-body p").each((_, el) => {
      ...
      if (/npsn/i.test(label)) npsn = value;
      else if (/status$/i.test(label)) status = value;
  ...
  let maleStudents: number | undefined = undefined;
  let femaleStudents: number | undefined = undefined;
  let totalTeachers: number | undefined = undefined;
  if (rekap && rekap.length) {
    ...
    if (pdText) totalStudents = parseNumber(pdText);
    ...
    if (maleText) maleStudents = parseNumber(maleText);
    if (femaleText) femaleStudents = parseNumber(femaleText);
    if (ptkTotal) totalTeachers = parseNumber(ptkTotal);
  }

  // Sarpras: count values inside table by reading numeric links
  const facilities: Array<{ name: string; count?: number }> = [];
  ...
  let kodePos: string | undefined = undefined;
  let kecamatan: string | undefined = undefined;
  let desa: string | undefined = undefined;
  ...
  if (kontak && kontak.length) {
    kontak.find(".panel-body p").each((_, el) => {
      ...
      const value = normalizeText(cloned.text()) || undefined;
      if (!label || !value) return;
      if (/alamat/i.test(label)) address = value;
      else if (/kode pos/i.test(label) || /kodepos/i.test(label)) kodePos = value;
      else if (/kecamatan/i.test(label)) kecamatan = value.replace(/^kec\.\s*/i, "");
      else if (/desa|kelurahan/i.test(label)) desa = value;
      else if (/lintang/i.test(label)) lat = value;
      else if (/bujur/i.test(label)) lon = value;
    });
  }
  ...
  const gradeStats: ScrapedSchoolData["gradeStats"] = [];
...
export async function scrapeSchool(entry: SchoolListEntry): Promise<ScrapeResult> {
...
export async function syncAllSchools(): Promise<SchoolCatalog> {
  ...
  const catalog: SchoolCatalog = {
    updatedAt: new Date().toISOString(),
    schools: results
      .filter((result) => result.success && result.payload)
      .map((result) => result.payload as ScrapedSchoolData),
    results,
  };
...
export async function loadCatalog(): Promise<SchoolCatalog> {
  await ensureOutputFile();
  const raw = await readFile(OUTPUT_FILE, "utf8");
  return JSON.parse(raw) as SchoolCatalog;
}

export async function getSchoolByNpsn(npsn: string): Promise<ScrapedSchoolData | undefined> {
